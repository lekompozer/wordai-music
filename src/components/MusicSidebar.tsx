...
export default function MusicSidebar({
    ...
    // Load local playlists from localStorage on mount
    useEffect(() => {
        if (typeof window === 'undefined') return;
        import('@/lib/localLibrary').then(({ getLocalPlaylists }) => {
    ...
    const handleAddLocalFiles = async () => {
        ...
        try {
            ...
            const name = `Local Files ${new Date().toLocaleDateString()}`;
            const plId = `lpl_${Date.now().toString(36)}`;
            const newPl = { id: plId, name, tracks, createdAt: Date.now() };
            const updated = [newPl, ...getLocalPlaylists()];
            saveLocalPlaylists(updated);
            setLocalPlaylists(updated);
            setExpandedLocalPlaylist(plId);
        } catch (e) {
    ...
    const handleAddLocalFolder = async () => {
        ...
        try {
            ...
            const plId = `lpl_${Date.now().toString(36)}`;
            const newPl = { id: plId, name: result.folderName, tracks: result.tracks, createdAt: Date.now() };
            const updated = [newPl, ...getLocalPlaylists()];
            saveLocalPlaylists(updated);
            setLocalPlaylists(updated);
            setExpandedLocalPlaylist(plId);
                ...
                <div className="flex gap-2">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className={`w-full pl-8 pr-3 py-2.5 rounded-2xl text-sm border backdrop-blur-sm ${inputCls} outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/15`}
                        />
                    </div>
                    <button
    ...
    const renderLocalTab = () => {
        ...
        return (
            <div className="flex flex-col flex-1 overflow-hidden">
                ...
                {isTauri && localPlaylists.length > 0 && (
                    <div className="flex-1 overflow-y-auto py-2">
                        {localPlaylists.map(pl => {
                            const isExp = expandedLocalPlaylist === pl.id;
                            return (
                                <div key={pl.id}>
                                    {/* Playlist row */}
                                    <div
                                        className={`flex items-center gap-2 px-3 py-2.5 cursor-pointer transition-all ${effectiveDark ? 'hover:bg-white/[0.06]' : 'hover:bg-slate-50'}`}
                                        onClick={() => setExpandedLocalPlaylist(isExp ? null : pl.id)}
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className={`text-sm font-medium truncate ${textPrimary}`}>{pl.name}</p>
                                            <p className={`text-xs ${textSec}`}>{pl.tracks.length} {isVietnamese ? 'bài' : 'tracks'}</p>
                                        </div>
                                        {pl.tracks.length > 0 && (
                                            <button
                                                onClick={e => {
                                                    e.stopPropagation();
                                                    onPlayTracks(pl.tracks.map(t => ({
                                        ...
                                        <button
                                            onClick={e => { e.stopPropagation(); handleDeleteLocalPlaylist(pl.id); }}
                                            className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${effectiveDark ? 'text-slate-500 hover:text-rose-400 hover:bg-white/[0.08]' : 'text-slate-400 hover:text-rose-500 hover:bg-slate-100'}`}
                                        >
                                            <Trash2 className="w-3 h-3" />
                                        </button>
                                        {isExp ? <ChevronDown className={`w-4 h-4 ${textSec}`} /> : <ChevronRight className={`w-4 h-4 ${textSec}`} />}
                                    ...
                                    {/* Track list */}
                                    {isExp && (
                                        <div className={`mx-3 mb-1 rounded-xl overflow-hidden ${effectiveDark ? 'bg-white/[0.04]' : 'bg-slate-50'}`}>
                                            {pl.tracks.map((t, i) => (
                                                <div
                                                    key={t.id}
                                                    className={`flex items-center gap-2 px-3 py-2 cursor-pointer transition-all ${effectiveDark ? 'hover:bg-white/[0.06]' : 'hover:bg-slate-100'}`}
                                                    onClick={() => {
                                                        onPlayTracks(pl.tracks.map(tr => ({
                                                ...
                                                >
                                                    <span className={`text-xs w-5 text-right flex-shrink-0 ${textSec}`}>{i + 1}</span>
                                                    <HardDrive className={`w-3 h-3 flex-shrink-0 ${effectiveDark ? 'text-indigo-400/60' : 'text-indigo-400'}`} />
                                                    <div className="flex-1 min-w-0">
                                                        <p className={`text-xs font-medium truncate ${textPrimary}`}>{t.title || t.filePath.split(/[\\/]/).pop()}</p>
                                                        {t.artist && <p className={`text-[10px] truncate ${textSec}`}>{t.artist}</p>}
                                                    </div>
                                                    {t.durationSec > 0 && (
                                                        <span className={`text-[10px] flex-shrink-0 ${textSec}`}>
